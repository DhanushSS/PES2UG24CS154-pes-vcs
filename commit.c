...
//
// PROVIDED functions: commit_parse, commit_serialize, commit_walk, head_read,
// head_update
...
int commit_serialize(const Commit *commit, void **data_out, size_t *len_out) {
  ...
  size_t n = 0;
  int written;
  written = snprintf(buf + n, sizeof(buf) - n, "tree %s\n", tree_hex);
  if (written > 0) n += (size_t)written;
  if (commit->has_parent) {
    hash_to_hex(&commit->parent, parent_hex);
    written = snprintf(buf + n, sizeof(buf) - n, "parent %s\n", parent_hex);
    if (written > 0) n += (size_t)written;
  }
  written = snprintf(buf + n, sizeof(buf) - n,
                "author %s %" PRIu64 "\n"
                "committer %s %" PRIu64 "\n"
                "\n"
                "%s",
                commit->author, commit->timestamp, commit->author,
